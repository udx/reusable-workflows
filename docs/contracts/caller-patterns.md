...
<!-- short: Canonical caller contract rules and prompt-shaped reusable workflow invocation patterns -->
...
- Humans: start with `Core Rules` and `Workflow Contract Matrix`
- LLM/agent retrieval: use `Common Caller Q&A` for prompt-shaped examples
- Programmatic checks: enforce `Contract-First Rule (No Invented Fields)` against declared `workflow_call` inputs/secrets
...
Prompt-shaped snippets for common reusable-workflow questions.
